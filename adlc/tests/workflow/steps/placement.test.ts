...
const mockAgents = {
    "placement-coordinator": { description: "mock", prompt: "mock" }
};
...
describe("runPlacement", () => {
    ...
    it("delegates to placement-coordinator agent", async () => {
        await runPlacement("Add plant watering feature", "/tmp/test", mockAgents);

        expect(queryCallLog).toHaveLength(1);
        expect(queryCallLog[0].options.agent).toBe("placement-coordinator");
    });

    it("passes feature description in the prompt", async () => {
        await runPlacement("Add plant watering schedule", "/tmp/test", mockAgents);
    ...
    it("forwards hooks to the coordinator", async () => {
        ...
        await runPlacement("Add watering", "/tmp/test", mockAgents, undefined, fakeHooks);
