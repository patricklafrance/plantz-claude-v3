...
describe("parseSliceDependencies", () => {
    ...
    it("parses a slice with no dependencies", () => {
        writeFileSync(join(slicesDir, "01-plant-list.md"), "# Slice 1 — Plant List\n\nBuild the plant list component.\n");

        const nodes = parseSliceDependencies(slicesDir);

        expect(nodes).toHaveLength(1);
        expect(nodes[0]).toEqual({
            filename: "01-plant-list.md",
            number: 1,
            name: "plant-list",
            dependsOn: []
        });
    ...
    it("parses a slice with dependencies", () => {
        writeFileSync(
            join(slicesDir, "04-care-schedule.md"),
    ...
    it("extracts slice numbers and names correctly from various filenames", () => {
        writeFileSync(join(slicesDir, "10-long-kebab-case-name.md"), "# Slice 10\n\nSome content.\n");

        const nodes = parseSliceDependencies(slicesDir);

        expect(nodes[0].number).toBe(10);
        expect(nodes[0].name).toBe("long-kebab-case-name");
        expect(nodes[0].filename).toBe("10-long-kebab-case-name.md");
    });
    ...
    it("ignores non-slice files in the directory", () => {
        ...
        writeFileSync(join(slicesDir, "01-feature.md"), "# Slice 1\n\nContent.\n");

        const nodes = parseSliceDependencies(slicesDir);

        expect(nodes).toHaveLength(1);
        expect(nodes[0].filename).toBe("01-feature.md");
    });
