...
```typescript
import LogRocket from "logrocket";
import { useLogRocketInstrumentationClient, useHoneycombInstrumentationClient } from "@workleap/telemetry/react";
...
const telemetryClient = initializeTelemetry("sg", {
    // ... platform configs
    verbose: isDev,
    loggers: [isDev ? new BrowserConsoleLogger() : new LogRocketLogger({ logLevel: LogLevel.information })]
});
```
...
// Structured logging
logger.withText("Processing order").withObject({ orderId: "123", items: 5 }).information();

// With error
logger.withText("Failed to save").withError(new Error("Network timeout")).error();
```
...
const telemetryClient = initializeTelemetry("wlp", {
    ...
    loggers: debugTelemetry ? [new BrowserConsoleLogger()] : [new LogRocketLogger({ logLevel: LogLevel.warning })]
});
```
