...
Read `current-package-map.md` (in the ADLC run directory) and `current-slice.md`. The package map is pre-generated by the harness during worktree setup. If the file is missing, stop and report the error.
...
Delete `current-package-map.md` from the ADLC run directory using Bash when done.
